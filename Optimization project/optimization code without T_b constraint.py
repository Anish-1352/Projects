...
def objective_function(x):
    ...
    K_v = (E_e * A_isolator) / T_r
    delta_v = (M / N_iso) * 9810 / K_v
    h = T - delta_v

    L0 = math.sqrt(T ** 2 - h ** 2)
    A_final = (B - (L - L0)) * (B - 0.3 * (L2 - L0))
    A_total = (A_isolator + A_final) / 2

    K_eff = (A_total * G_rubber) / T_r
    print("k_eff:", K_eff)
    ...
    S_ag_value = get_S_ag_value("medium", T_b)  # Assuming "medium" soil type

    Beta = 0.10 / (0.05 + B_eff)

    S_v = z * S_ag_value * Beta * 9810 * (T_b / (2 * math.pi))
    S_D = S_v * (T_b / (2 * math.pi))
    V_isolator = K_eff * S_D

    U = 0.5 * M * (S_v) ** 2  # Energy transferred to the superstructure or work done by the isolator
...
if __name__ == "__main__":
    ...
    # Run optimization
    optimization_result = minimize(objective_function, initial_guess, bounds=variable_bounds, method='L-BFGS-B', tol=1e-3,
